    ...
    <React.Fragment>
      ...
      <AdminSidebar side={side}/>
        ...
        <div className={`${categorycss.content} ${side ? '' : categorycss.noSidebar}`} >
          ...
          <form onSubmit={(e)=>handleSubmit(e)} style={{paddingTop:'57px'}}>

             <h2 className='text-center mb-5  '>Add category</h2>
             <div className={addCategoryCss.mainFormdiv}>
                       
                       <div>
                       {data.isParent ?<h4>Category name</h4>: <h4>Sub Category name</h4>}
                      <input onChange={(e)=>handleState(e)} value={data.categoryName} type="text" name="categoryName" id="" />
                      {error.categoryName && <h5 className='text-danger fs-6'>{error.categoryName}</h5> }
                      </div>

                      <div>
                        <h4>Is Subcategory</h4>
                      <input onClick={(e)=>setdatadispatch({type:'AddCategory',field:e.target.name,value:!data.isParent})}   type="checkbox" name="isParent" id="" /> </div>
                     ...
                     {!data.isParent && <div>
                      ...
                      <option value={0}>-----------</option>
                        ...
                        {mainCategory.map((obj)=>(
                             ...
                             <option  key={obj.id}  value={obj.id} >{obj.categoryName}</option>

                        ))}
                      </select>
                      {error.categoryParent && <h5 className='text-danger fs-6'>{error.categoryParent}</h5> }
                      
                      </div>}

                      <div>
                        <h4>Category image</h4>
                      <input onChange={(e)=>setdatadispatch({type:'AddCategory',field:e.target.name,value:e.target.files[0]})} accept="image/*" type="file" name="categoryImg"   /> 
                      {error.categoryImg  && <h5 className='text-danger fs-6'>{error.categoryImg}</h5> }
                       ...
                       <h4>About</h4>
                        ...
                        <textarea onChange={(e)=>handleState(e)} value={data.categoryAbout}  name="categoryAbout" id="">{data.categoryAbout}</textarea>
                        
                        {error.categoryAbout && <h5 className='text-danger fs-6'>{error.categoryAbout}</h5> }
