...
const Register = () => {
  ...
  const navigate = useNavigate()
   ...
   const handleSubmit=async(e)=>{
     ...
     try{
         ...
         const response = await axios.post('api/token/',{'Email':user.data['Email'],'password':val.password1})
  ...
  <div className={logincss.inpdiv} style={{top:0}}>
    ...
    <div>   <input type="text" name="First_name" value={val.First_name} onChange={(e)=>handlechange(e)} placeholder="First name" className={`w-100  ${logincss.inp}`} />
        
  {err.First_name &&   <h5 className={register.error}>{err.First_name}</h5> }
  </div>  
        
   <div>      <input type="text" name="Last_name" value={val.Last_name} onChange={(e)=>handlechange(e)} placeholder="Last name"  className={`w-100  ${logincss.inp}`}/>
  {err.Last_name &&   <h5 className={register.error}>{err.Last_name}</h5> }</div>
        
        <div>
         <input type="email" name="Email" value={val.Email} onChange={(e)=>handlechange(e)} placeholder="Email" className={`w-100  ${logincss.inp}`}/>
         
         {err.Email &&   <h5 className={register.error}>{err.Email}</h5> }
         {uniqeMailer && <h5 className={register.error}>{uniqeMailer}</h5>}</div> 
        
        <div>
         <textarea name="Address" id="" onChange={(e)=>handlechange(e)} className={`w-100  ${logincss.inp}`}  placeholder='Address'>{val.Address}</textarea>
         {err.Address &&   <h5 className={register.error}>{err.Address}</h5> }
         </div>
         <div style={{display:'flex',flexDirection:'row',justifyContent: 'space-around'}}>
         
         <input type="text"  name="Country" required value={val.Country} onChange={(e)=>handlechange(e)}  placeholder="Country"  className={`${logincss.inp} ${register.width40}`} />
         <input type="text" name="State" required value={val.State} onChange={(e)=>handlechange(e)}  placeholder="State"className={`${logincss.inp} ${register.width40}`}  />
         </div>
        
         <div style={{display:'flex',flexDirection:'row',justifyContent: 'space-around'}}>
         <input type="text" required name="District" value={val.District} onChange={(e)=>handlechange(e)}  placeholder="District"  className={`${logincss.inp} ${register.width40}`}  />
         <input type="number" required name="Postal_code" value={val.Postal_code} onChange={(e)=>handlechange(e)}  placeholder="Postal code"  className={`${logincss.inp} ${register.width40}`}  />
         </div>
         <div>
         <input type="number"  name="Phone" value={val.Phone} onChange={(e)=>handlechange(e)}  placeholder="Phone number" className={`w-100  ${logincss.inp}`} />
          {err.Phone && <h5 className={register.error}>{err.Phone}</h5>}
         </div>
        <div>
         <input type="password" value={val.password1} onChange={(e)=>handlechange(e)} name="password1"  className={`w-100  ${logincss.inp}`}  placeholder="Password"   />
         {err.password1 && <h5 className={register.error}>{err.password1}</h5> }
         </div>
         <div>

         <input type="password" value={val.password2} onChange={(e)=>handlechange(e)} name="password2" placeholder="Password Confirm"   className={`w-100  ${logincss.inp}`}  />
          {err.password2 && <h5 className={register.error}>{err.password2}</h5>}
        
        </div> <button type='submit' onClick={e=>handleSubmit(e)} className={logincss.button}>Submit</button>
...
<div onClick={()=>onGoogleLoginSuccess(false)} style={{position:'relative'}}>
